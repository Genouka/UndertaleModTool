...
public partial class UndertaleRoomViewModel : ObservableObject, IUndertaleResourceViewModel
{
    ...
    public UndertaleRoomViewModel(UndertaleRoom room, IServiceProvider serviceProvider)
    {
        ...
        if (!isGMS2)
            RoomTreeItems.Add(new(LocalizationSource.GetString("Room_Backgrounds"), "Backgrounds", Room.Backgrounds));

        RoomTreeItems.Add(new(LocalizationSource.GetString("Room_Views"), "Views", Room.Views));

        if (!isGMS2)
        {
            RoomTreeItems.Add(new(LocalizationSource.GetString("Room_GameObjects"), "GameObjects", Room.GameObjects));
            RoomTreeItems.Add(new(LocalizationSource.GetString("Room_Tiles"), "Tiles", Room.Tiles));
        }

        if (isGMS2)
            RoomTreeItems.Add(new(LocalizationSource.GetString("Room_Layers"), "Layers", Room.Layers));
    ...
    public async void SaveAsImage()
    {
        ...
        {
            Title = LocalizationSource.GetString("Msg_SaveImageTitle"),
