...
public partial class UndertaleSpriteViewModel : ObservableObject, IUndertaleResourceViewModel
{
    ...
    /// <summary>
    /// Binding-friendly accessor over the <see cref="UndertaleSprite.V3NineSlice"/> field, which
    /// cannot be bound to directly because it is a field rather than a property.
    /// </summary>
    public UndertaleSprite.NineSlice? SpriteNineSlice => Sprite?.V3NineSlice;
    ...
    public async void ExportAllTexturesAsPNGs()
    {
        ...
        {
            Title = LocalizationSource.GetString("Msg_ExportAllTexturesToFolder"),
        });
        ...
        {
            MessageWindow.Result result = await MainVM.View!.MessageDialog(LocalizationSource.GetString("Msg_ReplaceExistingFiles")
        ...
        for (int i = 0; i < Sprite.Textures.Count; i++)
        {
            ...
            if (file is null)
            {
                await MainVM.View!.MessageDialog(string.Format(LocalizationSource.GetString("Msg_ErrorCreateFile"), fileName));
                return;
            }
    ...
    public async void ImportCollisionMaskData()
    {
        ...
        {
            Title = LocalizationSource.GetString("Msg_ImportCollisionMaskData"),
    ...
    public async void ExportCollisionMaskData()
    {
        ...
        {
            Title = LocalizationSource.GetString("Msg_ExportCollisionMaskData"),
